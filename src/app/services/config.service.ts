...
export class ConfigService {
  ...
  getConfiguration(): DataConfiguration {
    ...
    resetConfiguration() {
      ...
      this.configSignal.set({
        ...
        transformation: {
          ...
          country: 'ECU', // Default country value
    ...
    exportJSON(): string {
      ...
      const finalTransformation: any = {
        ...
        properties: config.transformation.code === 'T010' ? [] : (autoGeneratedProperties.length > 0 ? autoGeneratedProperties : []),
        filter: config.transformation.code === 'T010' ? [] : config.transformation.filter
      };
      ...
      // Conditional logic based on transformation type

      if (config.transformation.code === 'T010') {

        // T010: Use simple structure with entities array directly
        ...
        // T012 or other: Use group structure with entitiesPattern + groupId
  ...
  deleteClientConfig(id: string): void {
    ...
    // 1. First delete all connections
    Promise.all(deletePromises)
      .then(() => {
        // 2. Then delete the ClientConfig from GraphQL
        return this.graphqlService.deleteClientConfig(id).toPromise();
      })
      .then(() => {
        console.log('ClientConfig y sus conexiones eliminados exitosamente:', id);
        // 3. Finally update the signal by removing this config
        this.clientConfigsSignal.update(configs =>
          configs.filter(c => c.id !== id)
        );
        // 4. Also remove connections from the graphqlConnections signal
        this.graphqlConnectionsSignal.update(connections =>
          connections.filter(conn => conn.clientName !== clientConfig.name)
        );
      })
      .catch(error => {
  ...
  private consolidateDuplicateClientConfigs(clientConfigs: any[]): any {
    ...
    configsByName.forEach((configs, name) => {
      // SOLO consolidar duplicados de MAXPOINT_LEGACY
      if (configs.length > 1 && name === 'MAXPOINT_LEGACY') {
        ...
        // Para cualquier otro cliente (no MAXPOINT_LEGACY), mantener TODOS sin consolidar
        configsToKeep.push(...configs);
      }
    });
